...
public class ReturnToGroupPosRelative : JobComponentSystem
{
    [BurstCompile]
    struct AddPosition : IJobForEach<BallGroupCenterComponent, Translation, ForceToAddComponent, RelativePosToLaunchGroup, ForceComponent>
    {
        public float returnToRelPosStrength;
        public void Execute([ReadOnly] ref BallGroupCenterComponent ballGroupCenter, [ReadOnly] ref Translation translation, ref ForceToAddComponent forceToAdd, [ReadOnly] ref RelativePosToLaunchGroup relativePosToLaunchGroup, [ReadOnly] ref ForceComponent forceComponent)
        {
            //if (Mathf.Approximately(returnToRelPosStrength, 0))
            //{
            //    return;
            //}
            ...
            Vector3 directionToAdd = ((ballGroupCenter.Value + relativePosToLaunchGroup.Value) - translation.Value);
//            Debug.Log(translation.Value  + "SDFSDFSDF");
            ...
            if (Vector3.Distance(translation.Value, ballGroupCenter.Value) > .1f)
            {
                ...
                Vector3 force = returnToRelPosStrength * ((shouldVelocity - (Vector3)(forceComponent.Value)).normalized *
                                              ...
                                              Mathf.Pow(Mathf.Clamp(Vector3.Distance(ballGroupCenter.Value, translation.Value), .3f, 5000),
                                                  1 / 3));
       ...
       // if (!Mathf.Approximately(PlayTimeSettings.returnToGroupPosRelative, 0))
        {
            ...
            var job = new AddPosition { returnToRelPosStrength = returnStrength };
